...
#             - matching_elements: List of elements with scores >= threshold
#     """
...
# def get_embedding_LLM_costs_for_KB(model_id: str, start_time: datetime, end_time: datetime, granularity_seconds_period: int = 10, assumed_GB_text = 1):
#     cloudwatch = boto3.client('cloudwatch')

#     namespace = 'AWS/Bedrock'  # Bedrock metrics are under this namespace
...
#     }
#     if 'titan-embed-text-v2:0' in model_id:
#         json_token['per million input token costs'] = 0.02

#     json_token['total token costs'] = json_token['per million input token costs'] * (json_token['input_tokens']/1000000)
    
#     # chars_per_token = 4
#     # tokens_in_assumed_GB_text = (assumed_GB_text*1024*1024*1024)/chars_per_token

#     # json_token[f'Assuming {assumed_GB_text} GB: total token costs'] = json_token['per million input token costs'] * (tokens_in_assumed_GB_text/1000000)    
    
#     return json_token

# def get_bedrock_tokens(model_id: str, start_time: datetime, end_time: datetime, granularity_seconds_period: int = 10):
#     cloudwatch = boto3.client('cloudwatch')
...
#     if 'titan-embed-text-v2:0' in model_id:
#         json_token['per million input token costs'] = 0.02
#         json_token['per million output token costs'] = 0
        
#     json_token['input token costs'] = json_token['per million input token costs'] * (json_token['input_tokens']/1000000)
#     json_token['output token costs'] = json_token['per million output token costs'] * (json_token['output_tokens']/1000000)
#     json_token['total token costs'] = json_token['input token costs'] + json_token['output token costs']
    
#     if json_token['invocation_count'] != 0:
#         json_token['average token costs per invocation'] = json_token['total token costs'] / json_token['invocation_count']
#     else :
#         json_token['average token costs per invocation'] = 0

#     json_token['token costs per MILLION such invocations'] = json_token['average token costs per invocation'] * 1000000
...
def get_embedding_LLM_costs_for_KB(
    ...
    json_token['input token costs'] = (
        json_token['per million input token costs'] * (json_token['input_tokens'] / 1_000_000)
    )
    json_token['output token costs'] = (
        json_token['per million output token costs'] * (json_token['output_tokens'] / 1_000_000)
    )
...
def get_bedrock_tokens(model_id: str, start_time: datetime, end_time: datetime, granularity_seconds_period: int = 5):
    """
