...
def create_bedrock_execution_role(role_handler, s3_bucket_name):
    ...
    role_name = (
        f"advanced-rag-workshop-bedrock_execution_role-{role_handler.region_name}"
    )
    ...
    try:
        ...
        print(
            f"Policy and roles have been created already. ARN: {bedrock_kb_execution_role_arn}"
        )
    except Exception as e:
        if "NoSuchEntity" in str(e):
            try:
                # Role does not exist, create it
                bedrock_kb_execution_role = role_handler.create_bedrock_execution_role(
                    s3_bucket_name
                )
                bedrock_kb_execution_role_arn = bedrock_kb_execution_role["Role"]["Arn"]
                print(
                    f"Created Bedrock Knowledge Base Execution Role ARN: {bedrock_kb_execution_role_arn}"
                )
            except Exception as e:
                print(e)
    ...
    if not bedrock_kb_execution_role_arn:
        ...
        bedrock_kb_execution_role_arn = (
            f"arn:aws:iam::{role_handler.account_number}:role/{role_name}"
        )
...
def create_cloudwatch_log_group(
    log_group_name, region_name="us-west-2", retention_days=None, tags=None
):
    """
                ...
                try:
                    ...
                    if tags is not None:
                        ...
                        logs_client.tag_log_group(
                            logGroupName=log_group_name, tags=merged_tags
                        )
...
def upload_directory(path, bucket_name, data_s3_prefix, region_name="us-west-2"):
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            ...
            s3.upload_file(
                os.path.join(root, file), bucket_name, key
            )  # Upload the file
...
def download_and_prepare_10k_reports(bucket_name, region_name="us-west-2"):
    ...
    # Define standard filenames to maintain consistency when loading data to Amazon S3
    filenames = ["Amazon-10k-2025.pdf", "Amazon-10k-2024.pdf", "Amazon-10k-2023.pdf"]
...
def download_and_prepare_csv_data(bucket_name, region_name="us-west-2"):
    ...
    response = requests.get(
        csv_url, verify=False
    )  # `verify=False` ignores SSL certificate issues
...
def generate_json_metadata(csv_file, content_fields, metadata_fields, excluded_fields):
        ...
        "documentStructureConfiguration": {
            ...
            "recordBasedStructureMetadata": {
                ...
                "metadataFieldsSpecification": {
                    "fieldsToInclude": [
                        {"fieldName": field} for field in metadata_fields
                    ],
                    "fieldsToExclude": [],
                },
            },
    ...
    json_data["documentStructureConfiguration"]["recordBasedStructureMetadata"][
        "metadataFieldsSpecification"
    ]["fieldsToExclude"] = [{"fieldName": field} for field in excluded_fields]
...
def create_opensearch_policies(
    iam_role_handler, vector_store_name, aoss_client, execution_role_arn
):
    """
...
def create_opensearch_access_policy(iam_role_handler, collection_id, execution_role):
    ...
    try:
        iam_role_handler.create_oss_policy_attach_bedrock_execution_role(
            collection_id=collection_id, bedrock_kb_execution_role=execution_role
        )
...
def create_oss_policy_attach_bedrock_execution_role(
    collection_id, bedrock_kb_execution_role, region_name, account_number
):
...
def create_opensearch_index(
    collection_host, index_name_prefix, region_name="us-west-2"
):
    """
    ...
    for strategy in ["fixed", "hierarchical", "semantic", "custom"]:
        ...
        try:
            ...
            print(
                f'Error while trying to create the index "{index_name}", with error {e.error}'
            )
...
def create_kb(kb_name, kb_description, kb_chunking_type, variables):
    ...
    try:
        ...
        get_kb_response = bedrock_agent.get_knowledge_base(
        ...
        with open("../variables.json", "w") as f:
            json.dump(
                variables, f, indent=4, default=str
            )  # Handle datetime serialization

        # Log knowledge base details
        print(
            f"OpenSearch Knowledge Response: {json.dumps(get_kb_response, indent=4, default=str)}"
        )
        ...
        if any(
            ...
            for phrase in ["already exist", "duplicate", "already been created"]
        ):
            ...
            for kb in list_kb_response.get("knowledgeBaseSummaries", []):
                ...
                if kb["name"] == kb_name:
                    ...
                    print(
                        f"Found existing knowledge base with Name: {kb_name} and ID: {kb_id}"
                    )
                    ...
                    print(
                        f'OpenSearch Knowledge Response: {json.dumps(get_kb_response["knowledgeBase"], indent=4, default=str)}'
                    )
                    break
        else:
    ...
    if kb is None:
        raise ValueError(
            f"Knowledge base '{kb_name}' not found or could not be created."
        )
...
def create_data_source_for_kb(chunking_strategy, data_source_name, kb, variables):
    ...
    try:
        print(
            f"Creating new data source '{data_source_name}' with {chunking_strategy_configuration} chunking..."
        )
...
def create_ingestion_job(kb, ds_object, variables):
    ...
    try:
        ...
        print(
            f"Ingestion job started successfully for kb_name = {kb['name']} "
...
def retrieve_from_kb(query, kb, n_chunks, variables, min_score=None):
    ...
    bedrock_agent_runtime = boto3.client(
        "bedrock-agent-runtime", region_name=variables["regionName"]
    )
    ...
    relevant_documents = bedrock_agent_runtime.retrieve(
        retrievalQuery={"text": query},  # The text query for retrieving documents
...
def analyze_chunk_scores_above_threshold(
    json_data, threshold, return_matching_elements=False
):
    """
...
BEDROCK_MODEL_PRICING = {
    ...
    "titan-embed-text-v2:0": {"input_per_million": 0.02, "output_per_million": 0.0},
...
def get_embedding_LLM_costs_for_KB(
    ...
    json_token["input token costs"] = json_token["per million input token costs"] * (
        json_token["input_tokens"] / 1_000_000
    )
    json_token["output token costs"] = json_token["per million output token costs"] * (
        json_token["output_tokens"] / 1_000_000
    )
    ...
    if json_token["invocation_count"] != 0:
        json_token["average token costs per invocation"] = (
            json_token["total token costs"] / json_token["invocation_count"]
        )
    else:
        json_token["average token costs per invocation"] = 0

    json_token["token costs per MILLION such invocations"] = (
        json_token["average token costs per invocation"] * 1_000_000
    )
...
def get_bedrock_tokens(
    ...
    start_time: datetime,
    end_time: datetime,
    granularity_seconds_period: int = 5,
):
    """
    ...
    json_token["input token costs"] = json_token["per million input token costs"] * (
        json_token["input_tokens"] / 1000000
    )
    json_token["output token costs"] = json_token["per million output token costs"] * (
        json_token["output_tokens"] / 1000000
    )
    json_token["total token costs"] = (
        json_token["input token costs"] + json_token["output token costs"]
    )
    ...
    json_token["token costs per MILLION such invocations"] = (
        json_token["average token costs per invocation"] * 1000000
    )
...
def create_or_update_custom_chunking_lambda(
    region_name, account_number, role_name, function_name, s3_bucket
):
    """
    ...
    function_arn = (
        f"arn:aws:lambda:{region_name}:{account_number}:function:{function_name}"
    )
...
def create_custom_data_source_for_kb(kb, variables, data_source_name, function_arn):
    ...
    custom_transformation_configuration = {
        "intermediateStorage": {
            "s3Location": {"uri": f"s3://{variables['s3Bucket']}-custom-chunk/"}
        },
        "transformations": [
            {
                "transformationFunction": {
                    "transformationLambdaConfiguration": {"lambdaArn": function_arn}
                },
    ...
    try:
        ...
        if existing_ds:
            print(f"Found existing data source '{data_source_name}'. Deleting it...")
            bedrock_agent.delete_data_source(
                knowledgeBaseId=kb_id, dataSourceId=existing_ds["dataSourceId"]
            )
            print("Waiting for data source deletion to complete...")
            time.sleep(20)
...
def get_default_generation_config(
    max_tokens=4096, temperature=0.2, top_p=0.5, stop_sequences=None
):
    """
...
def get_or_deploy_sagemaker_endpoint(
    model_id, model_version, instance_type, region_name="us-west-2"
):
    """
    ...
    if not llm_endpoint_name:
        try:
            ...
            llm_endpoint = llm_model.deploy(
                accept_eula=True, initial_instance_count=1, endpoint_name=endpoint_name
            )
            ...
            try:
                ...
                for endpoint in endpoints["Endpoints"]:
                    if model_id_suffix in endpoint["EndpointName"]:
                        llm_endpoint_name = endpoint["EndpointName"]
                        print(
                            f"Using existing endpoint as fallback: {llm_endpoint_name}"
                        )
                        break
            except Exception:
                pass
...
def create_sagemaker_llm(
    endpoint_name, generation_config, content_handler=None, region_name="us-west-2"
):
    """
...
def generate_response_from_sagemaker(
    prompt, endpoint_name, generation_config, region_name="us-west-2"
):
    """
...
def setup_and_run_rag_with_sagemaker(
    query,
    kb_id,
    endpoint_name,
    generation_config,
    ...
    # Generate response
    response = generate_response_from_sagemaker(
        prompt, endpoint_name, generation_config, region_name
    )

    return query, response


def save_sagemaker_endpoint_to_variables(
    variables, endpoint_name, file_path="../variables.json"
):
    """
...
def get_default_sagemaker_generation_config(
    temperature=0, top_k=10, max_new_tokens=512, stop="<|eot_id|>"
):
    """
...
def get_kb_config(
    kb_id: str, account_number: str, region_name: str, number_of_results: int = 5
) -> Dict[str, Any]:
    """
